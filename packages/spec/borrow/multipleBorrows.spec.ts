  ...
  describe('Given a user with collateral supplied to a reserve', () => {
    describe('When the user borrows from two different reserves', () => {
      ...
      it('Then the user has two active borrow positions with correct amounts', async () => {
        const firstBorrow = await findReserveToBorrow(client, user, {
          ...
          borrow(client, {
            ...
            amount: {
              erc20: {
                value: bigDecimal(
                  Number(reserve.userState!.borrowable.value.value) * 0.1,
                ),
              },
            },
          })
        ...
        expect(usdcPosition!.amount.value.value).toBeBigDecimalCloseTo(
          Number(firstBorrow.value.userState!.borrowable.value.value) * 0.1,
          2,
        );
        ...
        expect(usdsPosition!.amount.value.value).toBeBigDecimalCloseTo(
          Number(secondBorrow.value.userState!.borrowable.value.value) * 0.1,
          2,
        );
      });
    });
