...
import { useBorrowSwapQuote } from './useBorrowSwapQuote';
...
describe(`Given the '${useBorrowSwapQuote.name}' hook`, () => {
  ...
  describe('And a borrow swap quote request', () => {
    ...
    describe('When the request parameters change after an Accurate quote is shown', () => {
      it('Then it should surface the new Fast quote before the next Accurate quote arrives', async () => {
        const nextRequest: BorrowSwapQuoteRequest = {
          market: {
            ...request.market,
        ...
        const { result, rerender } = renderHookWithinContext(
          ({ quoteRequest }) => useBorrowSwapQuote(quoteRequest),
          {
